...
class CustomerHistoryAdapter (private val pagePosition: Int): ListAdapter<HistoryCustomer, CustomerHistoryAdapter.CustomerHistoryViewHolder>(DIFF_CALLBACK){
    ...
    override fun onBindViewHolder(
        ...
        if (pagePosition == 1) {
            ...
            holder.delete.setColorFilter(ContextCompat.getColor(holder.itemView.context, R.color.black), android.graphics.PorterDuff.Mode.SRC_IN)
        } else {
...
//        holder.totalPrice.text = (data.priceBarber).toString()
        holder.status.text = data.status
        GlobalScope.launch {
            val formattedPrice = data.priceBarber.toString().withCurrencyFormat()
